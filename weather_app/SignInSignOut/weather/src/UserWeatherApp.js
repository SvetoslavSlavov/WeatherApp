...
class UserWeatherApp extends Component {
...
componentWillMount() {
    this.City();
    axios.get('https://query.yahooapis.com/v1/public/yql?q=select%20*%20from%20weather.forecast%20where%20woeid%20in%20(select%20woeid%20from%20geo.places(1)%20%20where%20text%3D%22%27' + this.state.city + '%27%2C%20bg%22)%20and%20u%3D%27c%27&format=json&env=store%3A%2F%2Fdatatables.org%2Falltableswithkeys')
        .then(response => {
    ...
    City = () => {
        if (this.props.navigation.state.params.city === 'Sofia') {
            this.state.city = this.props.navigation.state.params.city;
        }
        if (this.props.navigation.state.params.city === 'Plovdiv') {
            this.state.city = this.props.navigation.state.params.city;
        }
        if (this.props.navigation.state.params.city === 'Varna') {
            this.state.city = this.props.navigation.state.params.city;
        }
    }
    // }
